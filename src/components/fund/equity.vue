<template>
	<div class="fund_content">
		...
		<div class="fund_item" v-for="(item, index) in equityList" :key="index" v-show="!isShowLoading">
		...
		<pagination :total="total" :current-page='pageIndex' :page-size="pageSize" @pageChange="getEquity" v-if="pageShow"></pagination>
	</div>
</template>
<script>
...
export default {
	...
	methods: {
		getEquity(currentPage){
			this.isShowLoading = true;
			fundApi.GetEquityList().then(res => {
				this.equityList = res;
