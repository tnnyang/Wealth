<template>
	<div class="fund_content">
		...
		<div class="insurance_item" v-for="(item, index) in demoesticList" :key="index" v-show="!isShowLoading">
		...
		<pagination :total="total" :current-page='pageIndex' :page-size="pageSize" @pageChange="getDomestic" v-if="pageShow"></pagination>
	</div>
</template>
<script>
...
export default {
	...
	methods: {
		getDomestic(currentPage){
			...
			insuranceApi.GetDomesticList().then(res => {
				this.demoesticList = res.slice(startPage, endPage);
